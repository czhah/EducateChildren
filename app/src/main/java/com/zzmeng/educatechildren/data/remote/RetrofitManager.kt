...
import com.zzmeng.educatechildren.EduApplication
...
class RetrofitManager {
    ...
    companion object {
        ...
        //  超时时间
        private val OUT_TIME = 10000L
        //  缓存大小
        private val NETWORK_CACHE_SIZE = 10 * 1024 * 1024L

        private val TYPE_FILE = MediaType.parse("multipart/form-data")
        ...
        private val cacheControlInterceptor = Interceptor { chain ->
            ...
            if (DatabaseConstant.isConnectedByState(EduApplication.INSTANCE?.applicationContext!!)) {
                //  没有网络 读取缓存
        ...
        fun getInstance(): Retrofit {
            synchronized(lock){
                if(INSTANCE == null) {
                    var cacheDir = File(EduApplication.INSTANCE?.cacheDir, "retrofit")

                    val cache = Cache(cacheDir, NETWORK_CACHE_SIZE)

                    val cookieJar = PersistentCookieJar(SetCookieCache(), SharedPrefsCookiePersistor(EduApplication.INSTANCE))
